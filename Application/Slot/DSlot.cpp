...
void DSlot::runFunction(void)
{
    ...
    while(runFlag == true)
    {
        ...
        if(os_error == (OS_ERR)OS_ERR_TIMEOUT)
        {
            ...
            //only have something to do if in running state, or init state
            ...
            else if(myState == E_SENSOR_STATUS_DISCOVERING)
            {
                sensorError = mySensor->initialise();
        ...
        {
            if((actualEvents & EV_FLAG_TASK_SLOT_POWER_DOWN) == EV_FLAG_TASK_SLOT_POWER_DOWN)
            {
 ...
 *  @return void
 */
void DSlot::powerDown(void)
{
    postEvent(EV_FLAG_TASK_SLOT_POWER_DOWN);
}
 ...
 *  @return void
 */
void DSlot::powerUp(void)
{
    postEvent(EV_FLAG_TASK_SLOT_POWER_UP);
}
...
bool DSlot::getValue(eValueIndex_t index, uint32_t *value)
{
    ...
    case E_VAL_INDEX_SAMPLE_TIMEOUT:
        *value = mySampleTimeout;
        break;
