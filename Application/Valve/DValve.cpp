...
#define CPU_CLOCK_FREQ 48000000u            // Hz
#define TMR_CLOCK_FREQ_TDM 1000000u         // Hz
#define TMR_CLOCK_FREQ_PWM 48000000u        // Hz
#define VALVE_PWM_FREQ 25000u               // Hz
#define VALVE_TDM_FREQ 500u                 // Hz
...
const uint32_t valveTdmPrescaler = (CPU_CLOCK_FREQ / TMR_CLOCK_FREQ_TDM) - 1u;   // Fixed divider is always 1, so subtract 1
const uint32_t valvePwmPrescaler = (CPU_CLOCK_FREQ / TMR_CLOCK_FREQ_PWM) - 1u;   // Fixed divider is always 1, so subtract 1
const uint32_t valveTdmReloadValue = (TMR_CLOCK_FREQ_TDM / VALVE_TDM_FREQ);      // timer clock / output frequency = counts
const uint32_t valveTdmPulseWidth = valveTdmReloadValue >> VALVE_DUTY_DIVIDER;        // Divide by 2 for 50 pc duty cycle
const uint32_t valvePwmReloadValue = (TMR_CLOCK_FREQ_PWM / VALVE_PWM_FREQ);      // timer clock / output frequency = counts
const uint32_t valvePwmPulseWidth = valvePwmReloadValue >> VALVE_DUTY_DIVIDER;        // Divide by 2 for 50 pc duty cycle
...
void DValve::calcOnePulseParameters(void)
{
    ...
    pulseCcr = 1000u;
#endif
    pulsePrescaler = valveTdmPrescaler;   // fixed
    pulseArr = valveTdmReloadValue;
    pulseCcr = valveTdmPulseWidth;
...
void DValve::calcPwmParameters(void)
{
    ...
    pwmCcr = 384u;
#endif
    pwmPrescaler = valvePwmPrescaler;
    pwmArr = valvePwmReloadValue;
    pwmCcr = valvePwmPulseWidth;
}
...
/**
* @brief    Disables the valve operation by setting the valve driver pin to low
* @param    void
* @retval   void
*/
...
/**
* @brief    Trigger Valve - This function checks whether a valve is configured in one pulse mode or pwm mode. If in one
            pulse mode, the valve direction is also checked and controlled before activating the pulse to the valve.
            If configured in PWM mode, the direction of the valve is not checked, but based on the state the PWM drive
            to the valve is enabled or disabled.
            All the signals whether one shot or pwm are generated by timers configured when creating valve objects.
            Firmware only enables / disables the timers
* @param    eValveState_t valveState
            VALVE_STATE_ON - Turns valve ON in PWM mode / Drives valve in one direction in Pulse mode
            VALVE_STATE_OFF - Turns valve OFF in PWM mode / Drives valve in opposite direction in pulse mode
* @retval   void
*/
...
/**
* @brief    This function is used to set the PWM / Pulse mode frequencies for the valve drive signal.
            It updates the corresponding timer reload and capture compare registers and sets the update flag to make
            the timers effective to new values from the next cycle.
* @param    uint32_t valveTime - in direct micro seconds in pulse mode e.g. for a 1 ms pulse value is 1000
            in percentage of duty cycle in pwm mode i.e. 200 for 20pc duty cycle
* @retval   void
*/
...
/**
* @brief    Starts or stops the timer PWM output based on the configuration of the valve
* @param    eValveState_t valveState -
            VALVE_STATE_ON - start PWM output
            VALVE_STATE_OFF - Inhibit PWM output
* @retval   void
*/
...
/**
* @brief    This function sets the duty cycle of the valve PWM output
* @param    uint32_t dutyCycle - Used as a percentage value x 10 to set a 20 pc duty cycle of the set frequency PWM
            signal, a value of 200 should be written to this function
* @retval   void
*/
void DValve::setValveDutyCycle(uint32_t dutyCycle)
{
    ...
    float32_t pcDuty = 0.0f;

    if((dutyCycle >= VALVE_MIN_DC_PWM_TDM) && (dutyCycle <= VALVE_MAX_DC_PWM_TDM))
    {
        pcDuty = (float32_t)(dutyCycle) * VALVE_DC_CONV_FACTOR;
    }
...
/**
* @brief    Configures the timer to run in One pulse mode
* @param    void
* @retval   void
*/
