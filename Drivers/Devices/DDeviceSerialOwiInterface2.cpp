...
/* Error handler instance parameter starts from 6101 to 6200 */
/* Typedefs ---------------------------------------------------------------------------------------------------------*/
 ...
 */
bool DDeviceSerialOwiInterface2::sendString(char *str, uint32_t buffSize)
{
    DLock is_on(&myMutex);
    sendOverUART4((uint8_t *)str, (uint32_t)strnlen_s(str, buffSize));
    return true;
}
