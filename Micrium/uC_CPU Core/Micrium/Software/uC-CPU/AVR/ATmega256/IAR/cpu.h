...
typedef  CPU_INT08U               CPU_STK;                      /* Defines CPU stack data type.                         */
...
#define  CPU_INT_DIS()         do { _CLI(); } while (0)                 /* Disable interrupts.                          */
#define  CPU_INT_EN()          do { _SEI(); } while (0)                 /* Enable  interrupts.                          */
#endif

#if     (CPU_CFG_CRITICAL_METHOD == CPU_CRITICAL_METHOD_STATUS_STK)
                                                                        /* Push    CPU status word & disable interrupts.*/
#define  CPU_INT_DIS()         do { _OPC(0x930A); \
                                    _OPC(0xB70F); \
                                    _CLI();       \
                                    _OPC(0x930F); \
                                    _OPC(0x9109); } while (0)
                                                                        /* Pop     CPU status word.                     */
#define  CPU_INT_EN()          do { _OPC(0x930A); \
                                    _OPC(0x910F); \
                                    _OPC(0xBF0F); \
                                    _OPC(0x9109); } while (0)
#endif
