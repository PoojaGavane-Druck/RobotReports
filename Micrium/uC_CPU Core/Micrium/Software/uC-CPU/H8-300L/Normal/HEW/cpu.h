...
#define  CPU_INT_DIS()         do { set_imask_ccr(1); } while (0)       /* Disable interrupts.                          */
#define  CPU_INT_EN()          do { set_imask_ccr(0); } while (0)       /* Enable  interrupts.                          */
#endif

#if     (CPU_CFG_CRITICAL_METHOD == CPU_CRITICAL_METHOD_STATUS_LOCAL)
                                                                        /* Save    CPU status word & disable interrupts.*/
#define  CPU_INT_DIS()         do { cpu_sr = get_ccr(); \
                                    set_imask_ccr(1);   } while (0)
#define  CPU_INT_EN()          do { set_ccr(cpu_sr);    } while (0)     /* Restore CPU status word.                     */
#endif
