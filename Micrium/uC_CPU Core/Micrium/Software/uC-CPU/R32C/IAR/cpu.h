...
#define  CPU_INT_DIS()         do { asm("FCLR I"); } while (0)          /* Disable interrupts.                          */
#define  CPU_INT_EN()          do { asm("FSET I"); } while (0)          /* Enable  interrupts.                          */
#endif

#if     (CPU_CFG_CRITICAL_METHOD == CPU_CRITICAL_METHOD_STATUS_STK)
                                                                        /* Push    CPU status word & disable interrupts.*/
#define  CPU_INT_DIS()         do { asm("PUSHC FLG"); \
                                    asm("FCLR I");    } while (0)
#define  CPU_INT_EN()          do { asm("POPC FLG");  } while (0)       /* Pop     CPU status word.                     */
#endif

#if     (CPU_CFG_CRITICAL_METHOD == CPU_CRITICAL_METHOD_STATUS_LOCAL)
                                                                        /* Save    CPU status word & disable interrupts.*/
#define  CPU_INT_DIS()         do { asm("STC FLG, $@", cpu_sr); \
                                    asm("FCLR I");              } while (0)
                                                                        /* Restore CPU status word.                     */
#define  CPU_INT_EN()          do { asm("LDC $@, FLG", cpu_sr); } while (0)
#endif
