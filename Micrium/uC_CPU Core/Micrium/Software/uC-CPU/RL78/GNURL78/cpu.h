...
#define  SET_INTERRUPT(cpu_sr)	__set_interrupt_state(cpu_sr)
#define  GET_INTERRUPT()        __get_interrupt_state()
...
#define  CPU_INT_DIS()         do { __asm("DI");  } while (0)   /* Disable interrupts.                          */
#define  CPU_INT_EN()          do { __asm("EI");  } while (0)   /* Enable  interrupts.                          */
#endif

#if     (CPU_CFG_CRITICAL_METHOD == CPU_CRITICAL_METHOD_STATUS_LOCAL)
                                                                        /* Save    CPU status word & disable interrupts.*/
#define  CPU_INT_DIS()         do { cpu_sr = GET_INTERRUPT(); \
                                    __asm("DI");            } while (0)
                                                                        /* Restore CPU status word.                     */
#define  CPU_INT_EN()          do { SET_INTERRUPT(cpu_sr);  } while (0)
#endif
...
CPU_INT08U  __get_interrupt_state();
void        __set_interrupt_state(CPU_INT08U);
