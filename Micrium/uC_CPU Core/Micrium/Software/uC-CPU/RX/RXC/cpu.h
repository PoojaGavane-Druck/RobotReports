...
typedef __evenaccess volatile  CPU_INT08U  CPU_REG08;           /*  8-bit register                                      */
typedef __evenaccess volatile  CPU_INT16U  CPU_REG16;           /* 16-bit register                                      */
typedef __evenaccess volatile  CPU_INT32U  CPU_REG32;           /* 32-bit register                                      */
typedef __evenaccess volatile  CPU_INT64U  CPU_REG64;           /* 64-bit register                                      */
...
*                                                                         octet @ lowest memory address)
*
*           (3) The default endianness can be overridden by setting the CPU_CFG_ENDIAN_TYPE inside
*               cpu_cfg.h.
*********************************************************************************************************
*/
...
*
*           (4) Interrupt mask level is set to CPU_CFG_KA_IPL_BOUNDARY to allow non-kernel aware ISRs at 
*               CPU_CFG_KA_IPL_BOUNDARY+1, CPU_CFG_KA_IPL_BOUNDARY+2 ... 15.
*********************************************************************************************************
*/

#ifndef CPU_CFG_KA_IPL_BOUNDARY                                 /* Configure default IPL boundary if undefined.         */
#define  CPU_CFG_KA_IPL_BOUNDARY  12u
#endif
...
#define  CPU_IntDis()          do { set_ipl(CPU_CFG_KA_IPL_BOUNDARY); } while (0)    /* Disable interrupts.             */
#else
#define  CPU_IntDis()          do { set_ipl(12); } while (0)
#endif
#define  CPU_IntEn()           do { set_ipl(0);  } while (0)                         /* Enable  interrupts.             */
...
#ifdef   CPU_CFG_KA_IPL_BOUNDARY
#define  CPU_INT_DIS()         do { cpu_sr = get_ipl(); \
                                    set_ipl(CPU_CFG_KA_IPL_BOUNDARY); } while (0)
#else
#define  CPU_INT_DIS()         do { cpu_sr = get_ipl(); \
                                    set_ipl(12); } while (0)
#endif
#define  CPU_INT_EN()          do { set_ipl(cpu_sr);    } while (0)     /* Restore CPU status word.                     */
#endif
