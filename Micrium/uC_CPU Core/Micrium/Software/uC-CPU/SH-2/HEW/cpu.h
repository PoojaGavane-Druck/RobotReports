...
#define  CPU_IntDis()          do { set_imask(15); } while (0)          /* Disable interrupts.                          */
#define  CPU_IntEn()           do { set_imask(0);  } while (0)          /* Enable  interrupts.                          */
...
#define  CPU_INT_DIS()         do { cpu_sr = get_cr(); \
                                    set_imask(15);     } while (0)
#define  CPU_INT_EN()          do { set_cr(cpu_sr);    } while (0)      /* Restore CPU status word.                     */
#endif
