...
# Apr 22
# updated to use updated getPVInfo() command with DK0499-ALGOV3-V6.hex

# May 06
# updated status codes to fix vented display
# updated to use PACE5000 for pump emulation
# with USB to RS232 cable SN FT1J3XMTA
# PACE5000 must be manually setup with appropriate slew rate and pressure type
# nominally 1 bar / s and units of barG

# May 27
# fixed bug that was setting sensor FS 1000 mbar lower for PM620 than PM620T
# fixed bug that was setting maximum applied pressure during automated testing to 1000 mbar
# lower than FS of sensor for PM620T
# updated pressure and vacuum only automated setpoints to remove low pass filter on setpoint values
# assumes a PACE will always be used to apply pressure/vacuum through the pump adaptor.

# June 8
# Added status flags excessOffset and excessVolume to display

import PySimpleGUIQt as sg
import dpi620gLib_newPV as dpi
import time
import random
...
PACE5000Port = []
if usePACE:
    # connect to PACE5000 for pump emulation
    PACE5000Port = PACE.findPACE5000('FT1J3XMTA')
    PACE.measurePACE5000(PACE5000Port=PACE5000Port)
...
layout = [
    [sg.Frame(layout=[
        ...
        [sg.Listbox(values=('Random Pressure and Vacuum',
                            'Random Pressure',
                            'Random Vacuum'),
                    size=(20, 2),
                    default_values='Random Pressure and Vacuum'),
...
window = sg.Window('PV624 GENII Simulator V8.0', layout, default_element_size=(200, 1), grab_anywhere=False)
...
if DPI620G:
    ...
    pressure, pv624Error, pv624Status, baroP = DPI620G.getPVInfo()
    DPI620G.setSetPoint(0)  # setpoint == atmospheric pressure
    minFS, maxFS, Ptype = DPI620G.getSensorInfo()
    FS = maxFS - minFS  # FS in mbar absolute
    ...
    try:
        while event != 'EXIT':
            ...
            if PACE5000Port:
                if action == 'Pump Up':
                    setpointPACE = round(FS / 1000, 3)
                    # set PACE to baroP above FS pressure of sensor
                    # FS is in mbar absolute and PACE is set to bar G units
                    PACE.controlPACE5000(PACE5000Port=PACE5000Port, setpoint=setpointPACE)
                    # print('PACE controlling to pressure ', setpointPACE, 'barG')

                elif action == 'Pump Down':
                    setpointPACE = round(-0.9)
                    PACE.controlPACE5000(PACE5000Port=PACE5000Port, setpoint=setpointPACE)
                    # print('PACE controlling to vacuum ', setpointPACE, 'barG')
                else:
                    PACE.measurePACE5000(PACE5000Port=PACE5000Port)
                    #print('PACE in Measure Mode')
            ...
            if event:
                if event == 'VENT':
                    ...
                    requestedSetpoint = int(values[2])  # in mbar G or A
                    requestedSetpointType = setPointTypes[values[3]]  # 0 == gauge, 1 == abs
                    if requestedSetpoint + 1000 * (1-requestedSetpointType) > FS:
                        print('Warning: requested setpoint exceeds FS of PM, limiting request')
                        requestedSetpoint = int(FS - 1000 * (1-requestedSetpointType))
                    #print(requestedSetpoint, requestedSetpointType)
                    #print(requestedSetpoint + baroP * (1-requestedSetpointType))
                    manualSetpoint = requestedSetpoint
                    manualSetpointType = requestedSetpointType
                    ...
                    if setpointMode == 'Random Pressure and Vacuum':
                        minPressureG = -850  # minimum gauge pressure of setPoint values
                        autoSetpointType = random.randint(0, 1)  # randomly do gage (0) or abs (1) pressure
                        # define a random setpoint in PMs FS range (mbar or mbar G), nominally -850 to FS mbar G
                        if autoSetpointType == 0:
                            # gauge pressure setpoint
                            autoSetpoint = random.randint(minPressureG, int(FS - baroP))
                        else:
                            # absolute pressure setpoint
                            autoSetpoint = random.randint(int(minPressureG + baroP), FS)
                    ...
                    if setpointMode == 'Random Pressure':
                        minPressureG = 30  # minimum gauge pressure of setPoint values
                        autoSetpointType = random.randint(0, 1)  # randomly do gage (0) or abs (1) pressure
                        # define a random setpoint in PMs FS range (mbar or mbar G), nominally -850 to FS mbar G
                        if autoSetpointType == 0:
                            # gauge pressure setpoint
                        ...
                        minPressureG = -850  # minimum gauge pressure of setPoint values (mbar G)
                        maxPressureG = -30  # maximum gauge pressure of setPoint values (mbar G)
                        autoSetpointType = random.randint(0, 1)  # randomly do gage (0) or abs (1) pressure
                        # define a random setpoint in PMs FS range (mbar or mbar G), nominally -850 to FS mbar G
                        if autoSetpointType == 0:
                            # gauge pressure setpoint
                            autoSetpoint = random.randint(minPressureG, maxPressureG)
                        else:
                            # absolute pressure setpoint
                            autoSetpoint = random.randint(int(minPressureG + baroP), int(maxPressureG + baroP))
                ...
                if setpointMode == 'Random Pressure' or \
                        setpointMode == 'Random Vacuum' or \
                        setpointMode == 'Random Pressure and Vacuum':
